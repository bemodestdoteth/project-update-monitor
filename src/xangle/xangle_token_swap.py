...
def insert(coins):
    con = sqlite3.connect('coins.db')
    cur = con.cursor()
    cur.execute("INSERT INTO xangle_token_swap VALUES (?, ?, ?, ?, ?)", coins)
    con.commit()
    con.close()
...
def xangle_token_swap_scrape(coin, driver, delay):
    ...
    if empty_database():
        ...
        con = sqlite3.connect('coins.db')
        cur = con.cursor()
        item_db = cur.execute("SELECT * FROM xangle_token_swap ORDER BY id DESC").fetchone()
        con.close()
        ...
        # Should only pass coins listed on Bithumb
            ...
            print_n_log(msg="A new coin {} swap disclosure has been discovered, buy it hasn't been listed on Bithumb.".format(item_site_comp[0]))
